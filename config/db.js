...
/**
 * Converts MySQL-style ? placeholders to SQLite-compatible ? placeholders.
 * Also converts MySQL-specific syntax to SQLite equivalents.
 */
function adaptSql(sql) {
  return sql
    // ON DUPLICATE KEY UPDATE → INSERT OR REPLACE (simplified handling)
    // For settings table: INSERT OR REPLACE
...
/**
 * Execute a query and return [rows, fields] just like mysql2/promise does.
...
// Pool-like object that wraps SQLite
const pool = {
  ...
  async getConnection() {
    // Return a connection-like object for transaction support
    return {
      async query(sql, params = []) {
        return executeQuery(sql, params);
      },
...
export async function initDb() {
  try {
    ...
    const existingAdmin = db.prepare("SELECT COUNT(*) as count FROM users WHERE username = ?").get('superadmin');
    ...
    if (heroCount.count === 0) {
      ...
      for (const s of slides) insertSlide.run(...s);
    }

    // Seed Events
    const eventCount = db.prepare("SELECT COUNT(*) as count FROM events").get();
    if (eventCount.count === 0) {
      const insertEvent = db.prepare(`INSERT INTO events (title, event_date, event_time, venue, category, description, cover_image, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
      const events = [
    ...
    const commCount = db.prepare("SELECT COUNT(*) as count FROM committee_members").get();
    if (commCount.count === 0) {
      const insertMember = db.prepare(`INSERT INTO committee_members (name, position, department, photo_url, bio, email) VALUES (?, ?, ?, ?, ?, ?)`);
      const members = [
    ...
    if (galleryCount.count === 0) {
      const insertGallery = db.prepare(`INSERT INTO gallery (title, category, media_url) VALUES (?, ?, ?)`);
      const gallery = [
    ...
    if (sponsorCount.count === 0) {
      const insertSponsor = db.prepare(`INSERT INTO sponsors (name, category, logo_url, website) VALUES (?, ?, ?, ?)`);
      const sponsors = [
      ...
      for (const s of sponsors) insertSponsor.run(...s);
    }

    // Seed News
    const newsCount = db.prepare("SELECT COUNT(*) as count FROM news").get();
    if (newsCount.count === 0) {
      const insertNews = db.prepare(`INSERT INTO news (title, slug, category, author, summary, content, featured_image, publish_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
      const news = [
    ...
    // Seed About Timeline
    const timelineCount = db.prepare("SELECT COUNT(*) as count FROM about_timeline").get();
    if (timelineCount.count === 0) {
      const insertTimeline = db.prepare(`INSERT INTO about_timeline (year, title, description, display_order) VALUES (?, ?, ?, ?)`);
      const timeline = [
      ...
      for (const t of timeline) insertTimeline.run(...t);
    }
