...
if ($method === 'GET') {
    $stmt = $pdo->query("SELECT * FROM history_milestones ORDER BY display_order ASC, id ASC");
    $rows = $stmt->fetchAll();

    json_response(true, 'Festival history milestones loaded successfully', $rows);
}
...
if ($method === 'POST') {
    ...
    if ($action === 'add' || (!isset($body['id']) && $action !== 'edit' && $action !== 'delete')) {
        ...
        $stmt = $pdo->prepare("INSERT INTO history_milestones (year, title, description, display_order) VALUES (?, ?, ?, ?)");
        $stmt->execute([$year, $title, $description, $display_order]);
        $id = $pdo->lastInsertId();

        json_response(true, 'History milestone created successfully', ['id' => $id]);
    }
    ...
    if ($action === 'edit' || (isset($body['id']) && $action !== 'delete')) {
        ...
        $stmt = $pdo->prepare("UPDATE history_milestones SET year = ?, title = ?, description = ?, display_order = ? WHERE id = ?");
        $stmt->execute([$year, $title, $description, $display_order, $id]);

        json_response(true, 'History milestone updated successfully', null);
    }
    ...
    if ($action === 'delete') {
        ...
        $pdo->prepare("DELETE FROM history_milestones WHERE id = ?")->execute([$id]);
        json_response(true, 'History milestone deleted successfully', null);
    }
}
